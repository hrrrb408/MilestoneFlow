...
/**
 * Audit integration tests for milestone complete/reopen operations.
 *
 * <p>Verifies that MILESTONE_COMPLETED and MILESTONE_REOPENED audit events
 * are written to the audit_event table with correct metadata.
 */
@DisplayName("Milestone Status Audit IT")
class MilestoneStatusAuditIT extends AbstractIntegrationTest {

    @Autowired private TestRestTemplate restTemplate;
    @Autowired private JdbcTemplate jdbc;
    @Autowired private PasswordEncoder passwordEncoder;

    private static final String EMAIL = "ms-status-audit@example.com";
    private static final String PASSWORD = "test-password-123";
    private static final String WS_SLUG = "ms-status-audit-ws";
    ...
    void setUp() {
        ...
        jdbc.update("""
            ...
            """, EMAIL, EMAIL.toLowerCase(), "Status Audit IT User", encodedPassword);
        ...
        var wsBody = Map.of("name", "Audit Status WS", "slug", WS_SLUG);
        ...
        authHeaders = buildAuthHeaders(accessToken);
        var projBody = Map.of("name", "Audit Status Project");
    ...
    @Test
    @DisplayName("should write MILESTONE_COMPLETED audit event on complete")
    void shouldWriteCompletedAuditEvent() {
        var body = Map.of("title", "Audit Complete Milestone");
        ...
        restTemplate.exchange(basePath + "/" + milestoneId + "/complete",
                HttpMethod.POST, new HttpEntity<>(authHeaders), Map.class);

        // Verify audit event
        List<Map<String, Object>> events = jdbc.queryForList(
                "SELECT action, target_type, target_id, workspace_id, actor_id, actor_type, metadata " +
                        "FROM audit_event WHERE action = 'MILESTONE_COMPLETED' AND actor_id = ?::uuid",
                userId);

        assertThat(events).hasSize(1);
        Map<String, Object> event = events.get(0);
        assertThat(event.get("action")).isEqualTo("MILESTONE_COMPLETED");
    ...
    @Test
    @DisplayName("should write MILESTONE_REOPENED audit event on reopen")
    void shouldWriteReopenedAuditEvent() {
        var body = Map.of("title", "Audit Reopen Milestone");
        String basePath = "/workspaces/" + workspaceId + "/projects/" + projectId + "/milestones";
        ResponseEntity<Map> createResponse = restTemplate.exchange(basePath, HttpMethod.POST,
                new HttpEntity<>(body, authHeaders), Map.class);
        @SuppressWarnings("unchecked")
        ...
        restTemplate.exchange(basePath + "/" + milestoneId + "/reopen",
                HttpMethod.POST, new HttpEntity<>(authHeaders), Map.class);

        // Verify audit event
        List<Map<String, Object>> events = jdbc.queryForList(
                "SELECT action, target_type, target_id, workspace_id, actor_id, actor_type " +
                        "FROM audit_event WHERE action = 'MILESTONE_REOPENED' AND actor_id = ?::uuid",
                userId);

        assertThat(events).hasSize(1);
        Map<String, Object> event = events.get(0);
        assertThat(event.get("action")).isEqualTo("MILESTONE_REOPENED");
        assertThat(event.get("target_type")).isEqualTo("MILESTONE");
        assertThat(event.get("target_id")).isEqualTo(UUID.fromString(milestoneId));
        assertThat(event.get("workspace_id")).isEqualTo(UUID.fromString(workspaceId));
        assertThat(event.get("actor_id")).isEqualTo(userId);
